...
* Take the return data from mysql select commands and create a specifically
  useful dictionary of results (if dictionary is the right data model for
  mysql results).
* Output to stdout json in the format Ansible is expecting in order to properly
  use the data.
...
parser.add_argument('--test', '-t', action='store_true',
                    help='Test output. Doesn\'t require mysql connection.')
group = parser.add_mutually_exclusive_group(required=True)
group.add_argument('--list', action='store_true', help='list all hosts')
group.add_argument('--host', action='store', help='list hostvars')
...
#
# thinking about it all the actions in these functions share some common
# elements such as the db connection handler etc. it might be a good
# candidate for a class.

def connect_to_db():
    """
    Create pymysql connection based off of db config file in the
    same directory.
    """
    ...
    if os.path.isfile("db.ini"):
        ...
        #TODO: figure out if a custom exception is needed or if I should
        # be capturing this exception in a specific way. this feels like
        # an opportunity for good error messaging to let the user know
        # that the applications config is incorrect.
...
def query_db(conn):
    ...
    take db connection handler and query db based on query within dbquery.ini
...
def list_all_machines(result):
    """
    This whole function bears quite a bit of explanation. The result of this
    function is a dictionary. It is intentional that the `for` loops below
    create a lot of seemingly redundant information in the dict it creates.
    This is an intentional decision to allow Ansible more finite and
    interesting filtering options when it eventually ingests the data.

    The mysql I use looks like:
    select assets.hostname, assets.customer, assets.category, assets.DC_cage,
    assets.model, assets.make
    ...
    for row in result:
        data.update({row[1]:{}})
        # data[str(row[1])] = {}  # machines.customer_id
        data[str(row[2])] = {}  # assets.category
        data[str(row[3])] = {}  # data_center_cages.name as 'DC_Cage'
        data[str(row[4])] = {}  # assets.model
        data[str(row[5])] = {}  # assets.make
        data[str(row[1])]['hosts'] = []
        data[str(row[2])]['hosts'] = []
        data[str(row[3])]['hosts'] = []
        data[str(row[4])]['hosts'] = []
        data[str(row[5])]['hosts'] = []
        if args.verbose:
    ...
    for x in dictlist:
        for row in result:
            if str(row[1]) == x:
                data[str(row[1])]['hosts'].append(row[0])
            if str(row[2]) == x:
                data[str(row[2])]['hosts'].append(row[0])
            if str(row[3]) == x:
                data[str(row[3])]['hosts'].append(row[0])
            if str(row[4]) == x:
                data[str(row[4])]['hosts'].append(row[0])
            if str(row[5]) == x:
                data[str(row[5])]['hosts'].append(row[0])
...
def main():
    if args.test:
        # Test output, bypasses DB connection and provides test set of
        # data to functions.
        result = (('server1', 'IBM', 'Server', 1, 'X1000', 'SuperMicro'),
                ('server2', 'BlueBox', 'Server', 1, 'Inspiron', 'Dell'),
                ('server3', 'IBM', 'Server', 1, 'X1000', 'SuperMicro'),
                ('Router1', 'BlueBox', 'Network', 1, 'SRX240', 'Juniper'),
                ('Router2', '', 'Network', 1, 'SRX550', 'Juniper'))
        test_bp_dict = list_all_machines(result)
        print(json.dumps(test_bp_dict, indent=4, sort_keys=True))
        sys.exit()

    conn = connect_to_db()
    if args.list:
        result = query_db(conn)
        bp_dict = list_all_machines(result)
